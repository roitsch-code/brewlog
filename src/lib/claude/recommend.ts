  ...
  const amountGuide: Record<string, string> = {
    ...
    custom: context.customWaterMl
      ? (context.occasion === "summer-time"
          ? `ICED + custom volume: the user typed ${context.customWaterMl}ml as the FINAL drink (hot brew + melted ice). Split it as ~60% hot brew / ~40% ice — so waterGrams (hot brew portion) should target ~${Math.round((context.customWaterMl ?? 350) * 0.6)}g (±30g), and the recipe's pour sequence describes pouring that much hot water onto ~${Math.round((context.customWaterMl ?? 350) * 0.4)}g of ice. Dose at 1:15 against the HOT brew portion (so ~${Math.round(((context.customWaterMl ?? 350) * 0.6) / 15)}g dose). BOTH candidates respect this split. Reference iced recipes from the corpus (Hoffmann Immersion Iced, AeroPress Iced) supply the TECHNIQUE — scale dose + water proportionally to land at this hot-brew target. Capacity tensions with the chosen method are handled separately (USER OVERRIDE block).`
