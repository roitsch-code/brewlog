  ...
  return (
    <div className="min-h-svh bg-transparent flex flex-col">
      ...
      <div className="px-5 pb-12 flex flex-col gap-8">
        ...
        {/* ─── FLAVOR PROFILE ─────────────────────────────────────
            The wheel is the visual identity of /taste — it stays above
            the fold. Top Flavors paired here so the wheel has its
            written readout next to it. */}
        ...
        {/* ─── COACH INSIGHTS ─────────────────────────────────────
            Each card = one multivariate observation + one suggestion.
            Generated by Opus over the full session corpus
            (lib/claude/insights.ts), cached server-side until a new
            brew lands. Default view shows the top 5; "Show more" reveals
            the rest (the corpus generates up to 8). */}
        <Section title="Coach">
          {insightsLoading ? (
            ...
            <div className="space-y-3">
              {(insightsExpanded ? insights : insights.slice(0, 5)).map((insight) => (
                <InsightCard
                  key={insight.id}
                  insight={insight}
              ...
              {insights.length > 5 && (
                <button
                  ...
                  className="w-full text-light-muted-foreground text-[12px] uppercase tracking-widest py-2 active:text-light-foreground"
                >
                  {insightsExpanded ? `Show fewer` : `Show ${insights.length - 5} more`}
                </button>
              )}
            </div>
          )}
