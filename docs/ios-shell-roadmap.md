...
- **Current phase:** Phases 1–3 done (`native/ios/` is on main). Phase 4 (`ios-testflight.yml`) **does NOT work for the first build** — see below. **The first iOS build moves to the owner's MacBook**, driven by a local Claude Code session with real Xcode access. CI returns for the monthly rebuilds once the project is proven + a shared scheme is committed.
- **Why the pivot (verified by inspecting the generated `native/ios/`, not from memory):** Capacitor 8 emits a **Swift Package Manager** project — `App.xcodeproj` only, **no `App.xcworkspace`** (so the testflight workflow's `-workspace App.xcworkspace` is wrong → must be `-project App.xcodeproj`); and the **`App` scheme is not committed** (cap's `native/ios/.gitignore` ignores `xcuserdata`, where the autocreated scheme lives), so a headless CI build can't find it. Fixing a Mac-only toolchain blind from a cloud runner was the wrong tool — the owner has a MacBook Pro and Xcode sees all of this.
- **Apple Developer enrollment status:** **ACTIVE (2026-06-15)**, 4 GitHub secrets added. App ID `com.roitsch.btts` + the App Store Connect app record: VERIFY they exist (needed for any TestFlight upload).
- **Next entry-point (on the Mac):** install Xcode (Mac App Store) → Xcode → Settings → Accounts → add the **private** Apple ID (roitsch@gmail.com) → install Claude Code on the Mac (VS Code "Claude Code" extension, or `claude` in Terminal) → open this repo → say **"build the iOS app"**. That session does: `cd native && npm install && npx cap sync ios`, then open `native/ios/App/App.xcodeproj` in Xcode (or `xcodebuild -project … -scheme App archive`), Archive → Distribute → TestFlight. Then **commit the shared scheme** (`App.xcodeproj/xcshareddata/xcschemes/App.xcscheme`) + fix `ios-testflight.yml` (`-project` not `-workspace`) so CI can resume the monthly rebuild.
