...
- **Current phase: 🟢 G4 TIER 2 WATCH APP — BUILD 6 UPLOADED, watch-app-install bug FIXED (2026-06-16).** Builds 4/5 had the watch app embedded but it NEVER appeared in the iPhone Watch app's "Verfügbare Apps" (owner confirmed: `1.0 (4)` installed, watchOS 26.5, no BTTS on the watch). Root cause found by inspecting the actual IPA: a **malformed Payload** — the watch app appeared TWICE, embedded in `App.app/Watch/` AND as a stray top-level `Payload/BTTSWatch.app`, because the watch target had `SKIP_INSTALL = NO`. App Store thinning then dropped the watch-app association. **Fix: `SKIP_INSTALL = YES`** in `native/scripts/add_watch_target.rb` (Stolperstein logged). Rebuilt as **build 6**, verified clean (`Payload/` = `App.app` only, watch embedded, both codesign-valid, versions 6/6), uploaded OK (Delivery UUID `c0cd9273-…`, no 90170). **Owner action now:** once build 6 finishes processing on TestFlight (~5–15 min), update to it in the TestFlight app → then iPhone **Watch app → "Verfügbare Apps" → BTTS → Installieren** (the watch app should now appear). Then open the BTTS watch app, start a brew, leave the phone screen ON → the wrist should buzz per step (the decisive feature).
