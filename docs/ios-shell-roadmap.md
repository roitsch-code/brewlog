...
- **Current phase: ✅ BUILD 3 (with HAPTICS) UPLOADED TO TESTFLIGHT (2026-06-15).** `UPLOAD SUCCEEDED` via `altool` (Delivery UUID `34c8e0b3-…`). This session shipped: (a) `gh auth login` done — all prior commits pushed (task #1 ✅); (b) lock-screen notifications made **alerting** (sound + vibration + watch haptic — PR #314); (c) **foreground Taptic haptics** (3-2-1 countdown + strong step buzz) + **agitation as discrete flow-rate-timed steps** (PR #315, web — visible on the PWA now); (d) the **`@capacitor/haptics` plugin** added to the shell + a **committed shared `App.xcscheme`** + **iPhone-only** device family + the build uploaded from the Mac. **Owner action now:** wait ~15 min for processing → TestFlight → update BTTS → start a brew → **it vibrates** (also Swirl/Stir/Tap, each at its flow-rate time). Lock-screen wasn't the real use case (wake-lock = no lock screen) — the foreground Taptic path is.
- **The working build recipe (Mac — bump `CURRENT_PROJECT_VERSION` each time):** prefix every `xcodebuild` with `GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=safe.bareRepository GIT_CONFIG_VALUE_0=all` (Mac global git has `safe.bareRepository=explicit`, blocks SPM). **(0) `cd native && npm install && npx cap sync ios && npm run assets`** (regenerate the AppIcon PNG — a clean checkout doesn't have it; see Stolperstein). (1) `xcodebuild archive -project native/ios/App/App.xcodeproj -scheme App -configuration Release -destination 'generic/platform=iOS' -archivePath /tmp/App.xcarchive CURRENT_PROJECT_VERSION=<N+1> CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY=""`; (2) `plutil -replace teamID -string <APPLE_TEAM_ID> native/exportOptions.plist` then `xcodebuild -exportArchive -archivePath /tmp/App.xcarchive -exportPath /tmp/export -exportOptionsPlist native/exportOptions.plist -allowProvisioningUpdates -authenticationKeyPath ~/.appstoreconnect/private_keys/AuthKey_A57ZL8HND3.p8 -authenticationKeyID A57ZL8HND3 -authenticationKeyIssuerID <ISSUER_ID>` (`git checkout native/exportOptions.plist` after — no team id in the repo); (3) `xcrun altool --upload-app -f /tmp/export/App.ipa -t ios --apiKey A57ZL8HND3 --apiIssuer <ISSUER_ID>`. **`<APPLE_TEAM_ID>` is the GitHub secret / decodable from the build's provisioning profile — it is NOT the `(…)` in the "Apple Development: …" keychain cert (that's the wrong personal team — see Stolperstein). `<ISSUER_ID>` is the ASC API issuer (owner has it; not findable on the Mac).**
