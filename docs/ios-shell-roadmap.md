...
- **Current phase: FIRST BUILD on the Mac, ~90% done — PAUSED mid-session (owner walking home, will resume on the Mac).** Verified working this session: unsigned **archive SUCCEEDS**, **export + distribution signing SUCCEEDS** (via the Admin API key), and **upload auth + the ASC app record resolve**. The ONLY remaining failure was the upload being rejected for **missing app icons** — now fixed (valid 1024px AppIcon generated + committed; the `native/.gitignore` bug that excluded `AppIcon-*.png` is fixed). Remaining = just rebuild → export → upload.
- **EXACT resume steps (Mac, repo root `/Users/M.Reuter/brewlog`, branch `claude/btts-ios-app-feasibility-95udin`; `native/` already has `npm install` + `cap sync` + `npm run assets` done):**
  - Every `xcodebuild` MUST be prefixed with `GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=safe.bareRepository GIT_CONFIG_VALUE_0=all` — the Mac's global git has `safe.bareRepository=explicit`, which blocks SPM from resolving (scoped override, do NOT change global git config).
  - **1. Unsigned archive:** `xcodebuild archive -project native/ios/App/App.xcodeproj -scheme App -configuration Release -destination 'generic/platform=iOS' -archivePath /tmp/App.xcarchive CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY=""`  (signing is deferred to export; automatic signing during archive demands a registered device — avoid it).
  - **2. Export (distribution-signed):** `plutil -replace teamID -string <TEAM_ID> native/exportOptions.plist` then `xcodebuild -exportArchive -archivePath /tmp/App.xcarchive -exportPath /tmp/export -exportOptionsPlist native/exportOptions.plist -allowProvisioningUpdates -authenticationKeyPath ~/Downloads/AuthKey_A57ZL8HND3.p8 -authenticationKeyID A57ZL8HND3 -authenticationKeyIssuerID <ISSUER_ID>`. Use the **Admin**-role key `A57ZL8HND3` (the earlier App-Manager key gave "Cloud signing permission error"). TEAM_ID + ISSUER_ID = the values in the GitHub secrets `APPLE_TEAM_ID` / `APP_STORE_CONNECT_ISSUER_ID` (ask owner; keep them out of this public repo).
  - **3. Upload:** `mkdir -p ~/.appstoreconnect/private_keys && cp ~/Downloads/AuthKey_A57ZL8HND3.p8 ~/.appstoreconnect/private_keys/` then `xcrun altool --upload-app -f /tmp/export/App.ipa -t ios --apiKey A57ZL8HND3 --apiIssuer <ISSUER_ID>`.
  - **4. After upload succeeds:** TestFlight → Internal Testing → add self → install on the iPhone. Then commit a SHARED scheme + change `ios-testflight.yml` `-workspace`→`-project` and have it run `npm run assets`, so CI resumes the monthly rebuild.
