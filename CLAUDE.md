...
8. **Retroactive audit when a Hard Rule is enacted.** This rule was added 2026-05-28 (commit `aa3019b`) to prevent **new** fabrications. The pre-existing codebase data fell into scope on enactment, but a full retroactive audit didn't run until commits `b9fc86b` (roaster purge), `d93a859`/`9684eef`/`793d9e7` (recipe ground-truth, 19 entries). When a rule is added, the audit must run **with** the rule, not weeks later. If you add a new Hard Rule, schedule the retroactive sweep before closing the commit.
