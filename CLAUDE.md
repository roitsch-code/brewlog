...
│   └── flowCoach.ts        # coachFlow(timeline, elapsed, started, liveGrams, samples) → one teaching cue ("Steady"/"Ease off"/"Overshot +Xg") comparing the Acaia weight stream to the intended flow. Coaches ANY step with a cumulative-grams target — percolation AND immersion/AeroPress pour steps (#411 dropped the old percolation-only gate); steps with no grams target (steep/press/drain) return "none". #443: selects the active step via the TIME-based `activeStepAt` (matching the card); the `settledGrams` median + the `weightedActiveIdx` step-pick were removed. PR #471: the caller (`LightStepBrew`) now passes **tare-netted** liveGrams/samples (raw scale minus the baseline captured at brew start), so an un-tared scale doesn't read the vessel weight as poured water. Tests: tests/dataflow/brew-flowcoach.test.mjs.
├── flow/
...
- **Residual overshoot (`+124.6g` / "pour 2 hit 180g 50s early") from vessel handling — pour flow now measured ONLY between Start and Done, vessel handling absorbed.** The tare baseline was captured at start, but the coach's rolling window accumulated pre-start setup samples AND any weight step from putting the carafe on the scale / lifting the dripper to swirl / taring mid-brew was read as poured water. Fix (`LightStepBrew.tsx` + new pure `src/lib/brew/tareTracker.ts`): `pushSample` drops every reading before the timer starts (setup is invisible), and `netWaterPoured()` converts the raw reading to water-poured by absorbing STEP-changes — anything ≥25 g moving faster than 60 g/s is vessel handling, not a kettle pour (≤~15 g/s), so it's folded into the baseline and `net` stays continuous. A slow BLE reconnect gap during a real pour (big step, but a pour *rate*) is NOT absorbed. Tests: `tests/dataflow/tare-tracker.test.mjs`.
