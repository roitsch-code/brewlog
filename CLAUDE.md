...
**Brew-timer rework: pour-timing math + weight on every step + monotonic step selection (June 2026, PRs #438 / #440)**
...
- **Weight-/pour-aware active step (`weightedActiveIdx`, `src/lib/utils/pourSequence.ts`).** The displayed pour step no longer advances purely on elapsed time — it KEEPS its card until the pour is actually finished (grams target reached on the Acaia, or its physical pour time elapsed off-scale), capped at the time index so it never races ahead. `coachFlow` picks the SAME step so the in-card numbers never drift from the card. **`#440` fix — MONOTONIC selection:** selection is driven off the **peak weight** seen this brew (a ref in `LightStepBrew`, reset on `elapsed===0`), NOT the instantaneous scale value. Without this a weight DIP (shifting thumb/cup pressure) un-completed a finished pour and the step JUMPED BACKWARDS (a #438 regression that also desynced the time-based watch + vibration → read as "watch/vibration broken"). The top `ScalePanel` still shows the raw live weight (drops normally); only step/progress selection is monotonic.
...
- **Wake lock for the whole brew screen.** Held on mount→unmount (gated to skip the cold-steep branch), not just once the timer starts at `elapsed===1`, so the screen never sleeps while you read/set up.
...
- **Architecture reminder:** all of the above is web code → reaches BOTH the iOS Capacitor remote-URL shell AND the Safari PWA the moment the Hetzner deploy finishes (no TestFlight rebuild). The native Apple-Watch app (build 19) only runs the step schedule the phone hands it via `useBrewStepWatch`/`brewWatch.ts`; this work didn't touch that path or the watch binary — only the boundary TIMES changed. **Open / unreproduced:** "Acaia connection worse" + "chat recipes show no pour guidance" were reported but could NOT be reproduced from the code (chat V60 + AeroPress both produce correct guidance through `buildBrewTimeline`); if they recur, a specific recipe is needed to repro. Tests: proportional-spacing + `weightedActiveIdx` cases in `pourSequence.test.mjs`.
...
6. **Palette — NOT a gap. The app is already colourful (do not describe it as "cream/bland").** The generative **Field** background paints a saturated fruity/floral gradient on *every* screen — the `src/lib/field/zones.ts` zones span fruity reds/oranges (hue 0–30), deep berry/magenta (350–375), and floral pinks (320–355), drifting via `FieldBlobs`. Cream is only the card-glass + base *under* that gradient; the dominant visual is the colour. (Owner confirmed June 2026, irritated at a stale "less cream pass not started" claim — that earlier note was wrong; this is the correction.) If a future *refinement* of specific cream surfaces (card glass / scrims) is wanted it would be foundational (touches `bg-light-card-default`, `gradientCreamScrim`, `light-*` tokens), but it is NOT an open gap and the app must never be called cream-dominant.
