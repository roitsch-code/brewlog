...
**Coach (`insights` + `coffeeInsight`) migrated Opus → Mistral Large (June 2026, PR #463)**
- **Why:** owner-requested follow-up to the `/recommend` migration ("Opus ist teuer, lass es uns wechseln"). After `/recommend` left Opus, these two were the only remaining Opus surfaces. Both are single-shot structured-JSON generations (no tool-use loop), so they port like `/recommend` did. The chat was deliberately left on Sonnet (see above).
...
- **Cost separation:** the coach uses its OWN key `MISTRAL_COACH_API_KEY` in a SEPARATE **"BrewLog-Coach"** Mistral workspace (Mistral isolates usage + billing per workspace) so coach spend bills apart from `/recommend`. Same "create the key INSIDE the right workspace" trap as recommend — switch workspace in the console FIRST, then mint the key.
- **Env / ops:** `MISTRAL_COACH_API_KEY` + `COACH_PROVIDER` in `docker-compose.yml` + `.env.example`; `deploy.yml` always-syncs BOTH Mistral keys into the VPS `.env` (guard raised to `wc -l > 2`). An empty `MISTRAL_COACH_API_KEY` secret → empty line → coach stays on Opus (auto-detect). **Activation:** add the GitHub secret, then trigger one deploy (bump `.deploy-trigger`) so the key reaches the VPS `.env`. **Rollback = `COACH_PROVIDER=anthropic`.** Owner-approved AI-behavior change (own commit/PR).
