...
│   ├── recipes/            # ★ Structured recipe corpus — 133 recipes total (10 championship + 72 reference + 51 experimental). championship.ts = WBrC/WAC champions (Kasuya 2016, Du 2019, Hsu 2022, Medina 2023, Wölfl 2024, Peng 2025, Stanica WAC 2024, …). reference.ts + expanded.ts = Hoffmann V60/Clever/AeroPress/Moccamaster/Iced, Kasuya 4:6, April V60 (Rolf), Gagné, Perger, Rao, Hatakeyama, Wallgren + dozens more. markusAdditions.ts = 51 user-supplied recipes (category "experimental"). Full pour mechanics with per-step durations, staged temperatures, attribution, sources, verified flag. selectRecipes() injected into /recommend per turn. **Equal best-match ranking (PR #193):** scoreRecipe() has NO pedigree/verified bonus — every recipe competes purely on context match (roast/process/variety/goal/occasion). **Method lock:** when the user locks a method in the flow (context.preferredMethod), selectRecipes hard-filters to that brewer (via brewersFromMethod) and returns the best N recipes FOR THAT METHOD; with no lock it's the one-per-brewer diversity portfolio. **Full mirror:** @./docs/recipes-full.md (all 133 with brew steps); @./docs/coffee-experts.md is the older 19-recipe summary (partially stale — recipes-full.md is canonical). (May 2026 source-audit: removed Turbo V60 — espresso-origin; replaced fabricated "Rolf Minimum Variables" with April's real house V60; corrected Perger to 12g/200g; flagged Hatakeyama numbers unsourced.)
...
- **Structured knowledge layer (May 2026)** — `src/lib/knowledge/{recipes,varieties,techniques}` carries the science/expertise corpus that backs `/recommend`, `/explore`, and `/explore-agent`. **Human-readable mirror:** @./docs/coffee-experts.md (mirror only — update the TS files first, then mirror). Replaces ad-hoc recipe paragraphs that were embedded directly in the prompts. **Recipes** = 133 total (10 championship + 72 reference + 51 experimental/user-supplied in `markusAdditions.ts`): WBrC/WAC champions (Kasuya 2016, Du 2019, Hsu 2022, Medina 2023, Wölfl 2024, Peng 2025, Stanica WAC 2024), Hoffmann V60/Clever/AeroPress/Moccamaster/Iced, Kasuya 4:6, April V60 (Rolf), Gagné, Perger, Rao, Hatakeyama Cafec, Wallgren Kalita-sieve, plus dozens more. Each entry: structured pour sequence with per-step durations and staged temperatures, attribution, sources, `verified` flag distinguishing canonical vs reconstructed details. Ranked equally on best-match (no pedigree/verified bonus); a locked method hard-filters selection to that brewer (see recipes/ note above). **Varieties** = ~25 WCR-grounded cultivar priors covering Bourbon family, Typica family, Ethiopian landraces, Geisha, SL series, F1 hybrids, Pacamara, Mokka. Genetic / agronomic facts sourced to the WCR Arabica Coffee Varieties Catalog; cup descriptions from Royal Coffee's *Green Coffee Book*. Pink Bourbon flagged per WCR's 2024 finding that it is genetically distinct from Bourbon despite the marketing name. **Techniques** = 18 atomic brewing moves (Hsu staged-temperature, Peng Melodrip, Wallgren sieving, Rao spin, water-first, low-mineral water, etc.) cross-referenced to exemplifying recipes — the brain can cite mechanism by id and reach a worked example. All three modules injected per turn; system prompts are NOT touched (cache hits preserved).
...
- **Chip — two documented sizes, pick by role:**
    - `default` (`px-4 py-2 text-[13px]`): "pick-one answer" rows where the chip IS the question's primary control. Sensory clarity/body/sweetness/bitterness/finish/flow on /brew log, SensoryToggle Yes/No, ImpressionRow primaries, roaster Q&A clarifications, the grinder picker on /onboarding.
    - `sm` (`px-3 py-1.5 text-[12px]`): multi-select tag rows, dense scrolling pickers, compact card-footer actions, secondary inline confirmations inside a denser review form. FlavorWheel quick picks, candidate role picker on /recommend, country picker, process / roast level on /brew scan review, coach actions on /taste and `CoachCard`, equipment multi-select on /onboarding.
    - Rule of thumb: chip stands alone as the answer to a focused question → `default`. One of many tags / dense row / secondary inline confirm → `sm`. Don't mix the two sizes on the same row. Both share the same glass treatment: `inline-flex items-center rounded-full font-medium leading-tight backdrop-blur-light-card backdrop-saturate-150 bg-light-card-default text-light-foreground`.
