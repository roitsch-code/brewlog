...
- **Scanned flavors auto-translate to English** (AI-behavior). Both scan paths — the vision bag scan (`analyzeBag.ts`) and the URL scrape (`analyze-url`) — return `tastingNotesFromBag` in English (e.g. French "Groseille, Rhubarbe, Agrumes" → "Redcurrant, Rhubarb, Citrus"), lining up with the English SCA taxonomy + Field-zone mapping. Still forbids inventing notes.
